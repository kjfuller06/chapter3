...
#   
#   if(any(!is.finite(g_temp$LFMC_max)) | any(!is.finite(g_temp$LFMC_min))){
#     g_temp2 = g_temp |>
#       filter(!is.finite(LFMC_max) | !is.finite(LFMC_min))
#     if(g_temp2$LFMC_sD2 != min(lfmc$date)){
#       print("LFMC values were Inf, attempting to extract earlier LFMC values")
#       lfmc_temp = lfmc |>
#         filter(date < g_temp2$LFMC_sD2[1])
#       lfmc_temp = lfmc_temp |>
#         filter(date == max(lfmc_temp$date))
#       
#       r = raster(lfmc_temp$files[1])
...
#       
#       if(any(is.finite(g_temp2$LFMC_max)) | any(is.finite(g_temp2$LFMC_min))){
#         print("Earlier LFMC value extracted, at least some values not Inf")
#         g_temp = g_temp |>
#           filter(is.finite(LFMC_max) & is.finite(LFMC_min)) |>
#           rbind(g_temp2)
#       } else {
#         print("Earlier LFMC values were also Inf")
#       }
#     }
#   }
#   # l[[i]] = g_temp
...
# crs(r) = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0")
# ## res 0.05
# g = st_transform(g, crs = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs+ towgs84=0,0,0"))
# ids = unique(g$ID)
# 
# counter <- 0
# l <- list(NULL)
