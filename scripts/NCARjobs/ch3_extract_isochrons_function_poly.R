...
gedi$TUF = as.numeric(difftime(gedi$time, gedi$DateTime, units = "days"))
gedi = gedi |> 
  filter(TUF > 0)

ext_progfun = function(x){
  gedi_temp = gedi[gedi$TUF < x & gedi$TUF > 0,]
  
  st_write(gedi_temp, paste0("ch3_isochrons_prefire", x, "_poly.gpkg"), delete_dsn = T)
}
ext_progfun(7)
ext_progfun(14)
ext_progfun(30)
