...
# first date ####
print(paste0("Iteration ", num , ": write raster for dates ", dates[num], " to ", dates[(num + 1)]))
...
distancefun = function(x){
  ...
  ## columns correspond to access features, rows correspond to systematically chosen points in the selected polygons
...
sfExport("distancefun", "access_temp", "iso_temp", "dates", "num")
sfLibrary(raster)
sfLibrary(sf)
sfLibrary(tidyverse)
sfLibrary(geojsonsf)
