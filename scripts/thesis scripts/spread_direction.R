    ...
    if(nrow(adj.start.temp[adj.start.temp$progtime != 0,]) > 0){
      ...
      if(!st_contains(start.poly, adj.poly, sparse = F)){
        ...
        tryCatch({
          ...
          max.adj = adj.pt[which(c.df$distance == max(c.df$distance)),] |> 
            distinct()
          max.start = start.pt[c.df$index[c.df$distance == max(c.df$distance)],] |> 
            distinct()
          
          sp.dir = unique(as.numeric(nngeo::st_azimuth(max.start, max.adj)))
          ...
          #   tm_shape(start.a) + tm_polygons(col = "darkblue") +
          #   tm_shape(adj.next) + tm_polygons(col = "darkred") +
          #   tm_shape(max.adj) + tm_dots() +
          #   tm_shape(max.start) + tm_dots() +
